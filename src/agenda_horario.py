...
from horas_dias_semana_data import verificar_horario
import time
...
class Agenda:
    ...
    def verificar_horario(self):
        verificar_hr = self.texto.split()
        horario = (verificar_hr[-1])

        if ('segundo' in horario.lower()) or ('minuto' in horario.lower()) or ('hora' in horario.lower()):
            ...
            tempo_atual = verificar_horario() # Chamada da função: Verificar_horario
